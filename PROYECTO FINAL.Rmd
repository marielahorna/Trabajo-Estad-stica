...
1. Con variables de control
```{r}
summary(datafin)
datatest=as.data.frame(datafin[,c(2:5,8,9)])
```
...
2. Sin variables de control:

```{r}
names(datatest)[2]="percapita"
names(datatest)[4]="desempleo"
str(datatest)

test_poisson1=glm(protestas~ percapita + gini + desempleo + postmateriales,family = poisson, data=datatest)

summary(test_poisson1)
```


```{r}
exp(confint(test_poisson1))
```

```{r}
#library(AER)
library(AER)
AER::dispersiontest(test_poisson1,trafo=1)
```
...
library(MASS)
str(datatest)
RBN= glm.nb(protestas~., data=datatest)
summary(RBN)
```
